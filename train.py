...
from data_preprocess import train_test_split
from derivative_language import DerivativeLanguage
...
def sentence_to_tensor(sentence, lang, isSrc):
    indexes = [lang.word2index[w] for w in lang.sentence_to_words(sentence, isSrc)]
...
def pairs_to_tensors(pairs, src_lang, trg_lang):
    tensors = [
        (sentence_to_tensor(src, src_lang, True), sentence_to_tensor(trg, trg_lang, False))
...
class Seq2Seq(pl.LightningModule):
    ...
    def predict(self, sentences, batch_size=128):
        ...
        pred_tensors = [
            sentence_to_tensor(sentence, self.src_lang, False)
